...
│
├── job                           # 定时任务模块
│   ├── FinancialArticleDataJob.java   # 新闻数据采集
│   ├── SectorDataJob.java             # 板块数据采集
│   └── DailyAnalysisJob.java          # 每日分析任务
│
└── common                        # 通用模块
    ├── config                    # 配置类
    └── util                      # 工具类
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  FinancialArticleDataJob          SectorDataJob             │
│  (每3小时)                         (每日16:00)              │
│       │                                │                    │
│       ↓                                ↓                    │
│  ┌─────────────┐               ┌───────────────┐            │
...
│  ┌─────────────────────────────────────────────┐            │
│  │              Elasticsearch                   │            │
│  │  news_data_items | sector_money_flow |       │            │
│  │                    sector_quote              │            │
│  └─────────────────────────────────────────────┘            │
└─────────────────────────────────────────────────────────────┘
```
