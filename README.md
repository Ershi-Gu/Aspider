...
| **混合检索** | 综合相关性搜索 | 0.6（归一化后） | 结合向量+关键词+业务规则，分数归一化到0-1范围 |
...
混合检索采用**固定基准归一化**方案，将原始分数归一化到0-1范围：

```java
// 归一化公式
normalizedScore = min(原始分数 / 20.0, 1.0)
```
...
| 归一化方式 | 公式 | 问题 |
|-----------|------|------|
| **相对归一化** | score / maxScore | 只反映批次内相对排名，不反映绝对相关性。查询"吃面"（不相关）也能得到0.98分 |
| **固定基准归一化** | score / 20.0 | 反映绝对相关性。查询"吃面"得0.14分，查询"电网"得0.8分 ✅ |

**分数示例**：

| 查询词 | 原始分数 | 归一化分数 | 是否通过阈值(0.6) |
|-------|---------|-----------|------------------|
| 电网（相关） | 12-18 | 0.6-0.9 | ✅ 通过 |
| 半导体（相关） | 10-16 | 0.5-0.8 | 部分通过 |
| 吃面（不相关） | 2-3 | 0.1-0.15 | ❌ 过滤 |
...
### 分析检索模块（AnalysisRetriever）

分析检索模块是Agent分析的数据供给层，负责并行检索新闻和板块数据，为后续Agent分析提供数据输入。
...
│                                                                  │
│   ┌─────────────────────┐       ┌─────────────────────────┐     │
│   │    NewsRetriever    │       │  SectorDataRetriever    │     │
│   │     新闻检索器       │       │    板块数据检索器        │     │
│   │  (多源融合+去重)     │       │   (趋势计算+排名)        │     │
│   └──────────┬──────────┘       └────────────┬────────────┘     │
│              │                               │                   │
...
│                                                                  │
│  ┌──────────────────┐  ┌──────────────────┐  ┌────────────────┐ │
│  │ 混合语义检索      │  │ 政策类新闻过滤    │  │ 高重要性过滤   │ │
│  │ (向量+关键词)     │  │ (NewsType=POLICY)│  │ (importance≥3)│ │
│  │ 基础分: 0.9      │  │ 基础分: 0.7      │  │ 基础分: 0.8   │ │
│  └────────┬─────────┘  └────────┬─────────┘  └───────┬────────┘ │
│           │                     │                    │          │
│           └─────────────────────┼────────────────────┘          │
│                                 ▼                               │
...
| 检索来源 | 基础分 | 衰减规则 | 说明 |
|---------|-------|---------|------|
| 混合语义检索 | 0.9 | 每下降1位 -0.01 | 语义相关性最高权重 |
| 高重要性新闻 | 0.8 | 每下降1位 -0.01 | 重要政策/事件优先 |
| 政策类新闻 | 0.7 | 每下降1位 -0.01 | 政策类新闻兜底 |

**去重逻辑**：同一文章在多个源出现时，保留评分最高的记录。

#### SectorDataRetriever 板块数据检索器
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. 解析查询参数（板块名称/代码 → 定位板块）                      │
│                        ↓                                         │
│  2. 获取当日资金流向 (todayFlow)                                 │
│     - 优先按名称查询，其次按代码查询                             │
│                        ↓                                         │
│  3. 获取近N日资金流向 (recentFlows)                              │
│     - 用于趋势分析，自动跳过非交易日                             │
│                        ↓                                         │
│  4. 获取当日行情 (todayQuote)                                    │
│     - 开盘价、收盘价、换手率等                                   │
│                        ↓                                         │
│  5. 计算排名 (inflowRank / totalSectors)                         │
│     - 当日主力净流入排名                                         │
│                        ↓                                         │
│  6. 计算趋势指标 (TrendIndicator)                                │
│     - 连续流入天数 / 累计流入金额 / 平均涨跌幅 / 趋势方向        │
└─────────────────────────────────────────────────────────────────┘
```
...
| 条件 | 趋势方向 |
|-----|---------|
| 平均涨跌幅 ≥ 2.0% 或 连续流入 ≥ 3天 | STRONG_UP |
| 平均涨跌幅 > 0.3% | UP |
| 平均涨跌幅 < -0.3% | DOWN |
| 平均涨跌幅 ≤ -2.0% | STRONG_DOWN |
| 其他 | NEUTRAL |

**连续流入天数计算**：查询 ES 时按 `tradeDate` 降序取最近 N 条记录，由于只有交易日才有数据，自动跳过周末和节假日。
...
│   │
│   ├── retriever/                           # 数据检索模块
│   │   ├── AnalysisRetriever.java               # 分析检索门面（并行编排）
│   │   ├── DataRetriever.java                   # 检索器接口
│   │   ├── NewsRetriever.java                   # 新闻检索器（多源融合）
│   │   ├── SectorDataRetriever.java             # 板块数据检索器（趋势计算）
│   │   └── domain/
│   │       ├── AnalysisQuery.java               # 分析查询请求
│   │       ├── AnalysisType.java                # 分析类型枚举
│   │       ├── NewsRetrievalResult.java         # 新闻检索结果
│   │       ├── RetrievalResult.java             # 综合检索结果
│   │       ├── RetrievalSource.java             # 检索来源枚举
│   │       ├── RetrievedArticle.java            # 带评分的新闻条目
│   │       ├── SectorDataResult.java            # 板块数据结果
│   │       ├── TrendDirection.java              # 趋势方向枚举
│   │       └── TrendIndicator.java              # 趋势指标
│   │
│   ├── agent/                               # Agent分析模块
│   │   ├── core/
│   │   │   ├── Agent.java                       # Agent接口
